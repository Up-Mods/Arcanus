	...
	public void effect(@Nullable LivingEntity caster, @Nullable Entity sourceEntity, Level world, HitResult target, List<SpellEffect> effects, ItemStack stack, double potency) {
		if(target.getType() == HitResult.Type.ENTITY) {
			...
			if(entityHit.getEntity() instanceof LivingEntity livingEntity)
				livingEntity.addEffect(new MobEffectInstance(ArcanusStatusEffects.BOUNCY.get(), ArcanusConfig.MovementEffects.BouncyEffectProperties.baseEffectDuration * (int) (effects.stream().filter(ArcanusSpellComponents.BOUNCY::is).count() * potency), 0, true, false));
		}
	}
}
