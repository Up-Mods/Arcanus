...
public record PocketDimensionPlot(UUID ownerId, BlockPos min, BlockPos max) {
	...
	public static PocketDimensionPlot fromNbt(CompoundTag tag) {
		return CODEC.decode(NbtOps.INSTANCE, tag).resultOrPartial(err -> FabricMain.LOGGER.error("Unable to decode PocketDimensionPlot from NBT: {}", err)).map(Pair::getFirst).orElse(null);
	}
